...
it(
  'exports related HTTP, Fastify, Nest and action spans and drains an in-flight subscription before closing the pool',
  {
    timeout: 60_000,
  },
  async () => {
    ...
    try {
      ...
      const subscribe = (subscriber: string) =>
      ...
      await waitUntil(
        () =>
          spans.some(
            (span) =>
              span.name === `POST ${SUBSCRIPTIONS_PATH}` && span.traceId === command.traceId,
          ),
