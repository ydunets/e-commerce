...
The existing client, server and migrations build matrix and semantic-release train remain in place. The versioned GitHub Release remains the human-facing release identity. Each build additionally records its exact GHCR digest, source commit, workflow run and attempt in an `image-<app>-<run>-<attempt>` artifact. Deployment checks that all three records belong to the same source and attempt, while semantic-release publishes a tag and GitHub release notes without creating a commit on protected `main`. Tags such as `latest` and package-version constants are not deployment inputs.
